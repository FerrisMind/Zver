...
impl AddressBar {
    ...
    /// 
    /// # Returns
    /// `Some(url)` if user requested to load a URL, `None` otherwise
    pub fn render(&mut self, ui: &mut egui::Ui) -> Option<String> {
        let mut load_url = None;

        ui.horizontal(|ui| {
            // DevTools toggle button
            let devtools_icon = if self.devtools_open { "▼" } else { "▶" };
            if ui.button(devtools_icon).on_hover_text("Toggle DevTools").clicked() {
            ...
            if response.lost_focus() && ui.input(|i| i.key_pressed(egui::Key::Enter))
                && !self.url_input.is_empty() {
                    load_url = Some(self.url_input.clone());
                }
